...
#%% Class(Plot) : all_vs_valid() ----------------------------------------------
    ...
    def all_vs_valid(self):
        ...
        # Info
        all_info = f"all ({len(x_m)})"
        valid_info = f"valid ({len(x_v_m)})"
    
        # Scatter plot
        
        ax0 = fig.add_subplot(gs[0, :2])
        ax0.scatter(x_m, y_m, label=all_info, s=5, alpha=0.5)
        ax0.scatter(x_v_m, y_v_m, label=valid_info, s=5, alpha=0.5)
        ...
        ax1.set_xticks([0, 1], [all_info, valid_info])
        ...
        ax2.set_xticks([0, 1], [all_info, valid_info])
...
#%% Class(Plot) : cond0_vs_cond1() --------------------------------------------
    ...
    def cond0_vs_cond1(self):
        ...
        # Info
        cond0_info = f"{'-'.join(self.tags_0)} ({len(x0_v_m)})"
        cond1_info = f"{'-'.join(self.tags_1)} ({len(x1_v_m)})"
        
        # Scatter plot
        
        ax0 = fig.add_subplot(gs[0, :2])
        ax0.scatter(x0_v_m, y0_v_m, label=cond0_info, s=5, alpha=0.5)
        ax0.scatter(x1_v_m, y1_v_m, label=cond1_info, s=5, alpha=0.5)
